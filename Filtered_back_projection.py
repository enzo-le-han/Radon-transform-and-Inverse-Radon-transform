...
# dimension
square_lenght = int(np.sqrt(len(data)))

# Create a 2D array of the grayscale values of the image.
image = np.array(data).reshape((square_lenght, square_lenght))
...
def radon(image, Nangles):
    M, N = image.shape # lenght image
    Nprojection = int(np.sqrt(M**2+N**2))+1 # lenght of diagonal
...
def filter_function(lenght,filter):

    n = np.concatenate(
        (
            # Generate a sequence of odd numbers ranging from 1 to (lenght/2 + 1) (exclusive), with a step of 2. This sequence represents the low spatial frequencies.
            np.arange(1, lenght / 2 + 1, 2, dtype=int),
            # Generate a sequence of odd numbers ranging from (lenght/2 - 1) to 1 (exclusive), with a step of -2. This sequence represents the high spatial frequencies.
            np.arange(lenght / 2 - 1, 0, -2, dtype=int),
        ))
    ...
    if filter == "ramp":
        ...
        omega = np.pi * fftfreq(lenght)[1:]
        fourier_filter[1:] *= np.sin(omega) / omega

    elif filter == "cosine":
        freq = np.linspace(0, np.pi, lenght, endpoint=False)
        cosine_filter = fftshift(np.sin(freq))
        fourier_filter *= cosine_filter

    elif filter == "hamming":
        fourier_filter *= fftshift(np.hamming(lenght))

    elif filter == "hann":
        fourier_filter *= fftshift(np.hanning(lenght))
...
def inverse_radon(image_radon,vector_theta,filter):
    ...
    image_radon_lenght = image_radon.shape[0]
    dtype = image_radon.dtype

    # Ensure that the resulting size will be a power of two, which is optimal for the FFT.
    lenght_projection = max(64, int(2 ** np.ceil(np.log2(2 * image_radon_lenght))))
    width = ((0, lenght_projection - image_radon_lenght), (0, 0)) # Number of rows to add until the specified size is reached.
    img = np.pad(image_radon, width, mode='constant', constant_values=0) # Fill the new rows with zeros to reach the power of two.

    # Apply filter
    fourier_filter = filter_function(lenght_projection,filter)
    projection = fft(img, axis=0) * fourier_filter
    radon_filtered = np.real(ifft(projection, axis=0)[:image_radon_lenght, :])
    
    # The rest of the code is similar to the radon() function, except for the inversion of the last line.
    new_lenght = int(image_radon_lenght/np.sqrt(2))
    reconstructed_image = np.zeros((new_lenght,new_lenght), dtype=dtype)

    cos = np.cos(vector_theta*np.pi/180)
    sin = np.sin(vector_theta*np.pi/180)

    q_valeurs = np.arange(0,image_radon_lenght)

    Xcentre = new_lenght/2
    Ycentre = new_lenght/2

    facteur = new_lenght*np.sqrt(0.5)
    ...
    for t in range(len(vector_theta)):
        ...
        for p in range(-int(image_radon_lenght/2),int(image_radon_lenght/2)):         
            x_val = np.round(x0 + q_valeurs*cost - p*sint).astype(int)
            y_val = np.round(y0 + p*cost + q_valeurs*sint).astype(int)

            indices_valides = (x_val >= 0) & (x_val < new_lenght) & (y_val >= 0) & (y_val < new_lenght)

            reconstructed_image[y_val[indices_valides], x_val[indices_valides]] += radon_filtered[p+int(image_radon_lenght/2), t]
